...
uintptr_t handle_trap(uintptr_t cause, uintptr_t epc, uintptr_t regs[32])
{
    ...
    printf("handle_trap do: cause = %x\n", cause);
    printf("handle_trap do: epc = %x\n", epc);
    write_csr(mip, 0);
    if ((cause == CAUSE_USER_ECALL) ||
        ...
        *CLINT_MTIME_H = 0;
        ...
        printf("process CAUSE_MACHINE_EXTERNAL_INTERRUPT\n");
        printf("pending = %x\n", *PLIC_PENDING);
        ...
        if ((int_id == PLIC_INT_ID_DMA_DONE(0)) | (int_id == PLIC_INT_ID_DMA_ERROR(0))) { //dma channel0
            ...
            if ((control & 0x80000000) != 0) {
                uint8_t *dest_ptr;
                printf("DMA_CHN0 control.error set\n");
                //clean error
        ...
        if (int_id == PLIC_INT_ID_UART0) { //uart0
            ...
            printf("UART0 ip = %x\n", ip);
            if ((ip & 0x2) != 0) {
                printf("UART0 ip.rxwm pending\n");
                //printf("UART0 rx data: %x\n", uart0_rx());
            }
            if ((ip & 0x4) != 0) {
                uint32_t error_code = *UART0_ERROR;
                printf("UART0 data parity error. code = %d\n", error_code);
        ...
        if (int_id == PLIC_INT_ID_SPI0) { //spi0
            ...
            printf("SPI0 ip = %x\n", ip);
            if ((ip & 0x2) != 0) {
                printf("SPI0 ip.rxwm pending\n");
                //printf("SPI0 rx data: %x\n", spi0_rx());
            }
        }
        ...
        *PLIC_CLAIM_COMPLETE_M(hart_id) = int_id; //complete
        ...
        printf("process CAUSE_SUPERVISOR_EXTERNAL_INTERRUPT\n");
        printf("pending = %x\n", *PLIC_PENDING);
