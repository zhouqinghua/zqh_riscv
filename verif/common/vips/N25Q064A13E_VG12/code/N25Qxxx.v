...
parameter [100*8:1] memory_file = `FILENAME_mem;

// parameter [2048*8:1] fdp_file = "sfdp.vmf";
parameter [100*8:1] fdp_file = `FILENAME_sfdp;
...
//tmp module PLRSpart1Detect (S, C, EN4b);
//tmp 
//tmp     parameter integer count = 24;
//tmp     parameter [8*8:1] protocol = "extended";
//tmp 
//tmp     input S;
//tmp     input C;
//tmp     input EN4b;
//tmp 
//tmp     integer iXIP_count;
//tmp     integer plrs_part1_excess_clock = 0;
//tmp     reg XIP_rst = 0;
//tmp     reg start_XIP_rescue = 0;
//tmp     reg not_valid_1st_part = 0;
//tmp     reg power_rst = 0;
//tmp     
//tmp     event plrs_part1_count_reached;
//tmp 
//tmp     always @N25Qxxx.startCUIdec if ((N25Qxxx.cmd[6] && N25Qxxx.cmd[4] && N25Qxxx.cmd[2] && N25Qxxx.cmd[0] && N25Qxxx.protocol=="dual") ||
//tmp                                     (N25Qxxx.cmd[7] && N25Qxxx.cmd[4] && N25Qxxx.cmd[3] && N25Qxxx.cmd[0] && N25Qxxx.protocol=="quad") ||
//tmp                                     (N25Qxxx.cmd=='b11111111 && N25Qxxx.protocol=="extended")) begin
//tmp         N25Qxxx.cmdRecName = "Power Loss Rescue Sequence";
//tmp         //$display("[%0t ns] COMMAND RECOGNIZED: %0s.", $time, N25Qxxx.cmdRecName);
//tmp         -> N25Qxxx.seqRecognized; 
//tmp     end    
//tmp 
//tmp     always @(negedge S) begin
//tmp         //if(protocol == N25Qxxx.protocol && power_rst == 0) begin
//tmp         if(protocol == N25Qxxx.protocol && EN4b) begin
//tmp             iXIP_count = count;  
//tmp             //$display("  [%0t ps] PLRS: 1st part of Power Loss Rescue Sequence initiated. ", $time);
//tmp             start_XIP_rescue = 1;
//tmp             plrs_part1_excess_clock = 0; 
//tmp         end
//tmp     end
//tmp     
//tmp     always @(posedge C) begin
//tmp     if (start_XIP_rescue ==  1 ) begin : Resc_seq
//tmp        if(iXIP_count > 0) begin
//tmp           iXIP_count = iXIP_count - 1;
//tmp             `ifdef HOLD_pin
//tmp                 if (N25Qxxx.DQ0 != 1 || N25Qxxx.HOLD_DQ3 != 1) begin
//tmp             `else
//tmp                 if (N25Qxxx.DQ0 != 1 || N25Qxxx.RESET_DQ3 != 1) begin
//tmp             `endif
//tmp              //$display("  [%0t ps] ***WARNING*** PLRS: 1st part of Power Loss Rescue Sequence to get memory out of XIP mode aborted ", $time);
//tmp              //$display("  [%0t ps] ***WARNING*** PLRS: Need to drive DQ0 and DQ3 to 1 !!! ", $time);
//tmp     //         iXIP_count = 0;
//tmp              //latchingMode = "N";
//tmp              disable Resc_seq;
//tmp           end
//tmp        end else begin
//tmp            if(S == 0)
//tmp             -> plrs_part1_count_reached;
//tmp             end 
//tmp        end         
//tmp     end // end for always block
//tmp 
//tmp     // detects that enough clock cycles for 1st part of PLR Sequence
//tmp     // had been given, we then wait for rising edge of S.
//tmp     //--------------------------------------------------------------
//tmp     always @plrs_part1_count_reached begin
//tmp         @(posedge S) begin
//tmp             if(not_valid_1st_part == 0) begin
//tmp                 iXIP_count = 0;
//tmp                 XIP_rst = 0;
//tmp                 //N25Qxxx.XIP = 0;
//tmp                 //VolatileReg.VCR[3] = 1;
//tmp                 start_XIP_rescue = 0;
//tmp               //$display("  [%0t ps] ==INFO== PLRS: 1st part of Power Loss Rescue Sequence to get memory out of XIP mode completed", $time); 
//tmp             end else begin
//tmp              not_valid_1st_part = 0;
//tmp              plrs_part1_excess_clock = 0;
//tmp              end
//tmp         end
//tmp     end
//tmp 
//tmp     // checks for excess clock pulses once the required clock pulses
//tmp     // have been reached for the 1st part of PLR Sequence
//tmp     //---------------------------------------------------------------
//tmp     always @plrs_part1_count_reached begin
//tmp         plrs_part1_excess_clock = plrs_part1_excess_clock + 1;
//tmp         if(plrs_part1_excess_clock > 1) begin
//tmp             //$display("   [%0t ps] ==ERROR== Excess clock cycles for 1st part of PLRS ", $time);
//tmp             not_valid_1st_part = 1;
//tmp         end    
//tmp     end
//tmp 
//tmp endmodule // PLRSpart1Detect 
...
//tmp module PLRSpart2Detect (S, C, modify_in_progress);
//tmp 
//tmp     parameter integer count = 7;
//tmp 
//tmp     input S;
//tmp     input C;
//tmp     input modify_in_progress;
//tmp     reg start_PLR2_rescue = 0;
//tmp     reg not_valid_2nd_part = 0;
//tmp     reg abort = 0;
//tmp     reg plr2_done = 0;
//tmp     integer iPow_res = 4;
//tmp     integer plrs_part2_excess_clock = 0; 
//tmp 
//tmp     event plrs_part2_count_reached;
//tmp 
//tmp     always @N25Qxxx.startCUIdec if ((N25Qxxx.cmd[6] && N25Qxxx.cmd[4] && N25Qxxx.cmd[2] && N25Qxxx.cmd[0] && N25Qxxx.protocol=="dual") ||
//tmp                                     (N25Qxxx.cmd[7] && N25Qxxx.cmd[4] && N25Qxxx.cmd[3] && N25Qxxx.cmd[0] && N25Qxxx.protocol=="quad") ||
//tmp                                     (N25Qxxx.cmd=='b11111111 && N25Qxxx.protocol=="extended")) begin
...
//tmp 
//tmp     always @(negedge S) begin
//tmp         `ifdef INT_RST_SEQ
//tmp              iPow_res = 16;
//tmp         `else
//tmp              iPow_res = count;
//tmp         `endif
//tmp              start_PLR2_rescue = 1;
//tmp          plr2_done = 0;
//tmp          plrs_part2_excess_clock = 0;
//tmp     end    
//tmp 
//tmp     always @(posedge C) begin
//tmp         if (start_PLR2_rescue) begin : Resc2_seq
//tmp             if (iPow_res > 0) begin : Power_rescue_seq
//tmp                 `ifdef HOLD_pin
//tmp                     if (N25Qxxx.DQ0 != 1 || N25Qxxx.HOLD_DQ3 != 1) begin
//tmp                 `else
//tmp                     if (N25Qxxx.DQ0 != 1 || N25Qxxx.RESET_DQ3 != 1) begin
//tmp                 `endif
//tmp                         //$display("  [%0t ps] ***WARNING*** PLRS: 2nd part of Rescue Sequence Aborted ", $time);
//tmp                         //$display("  [%0t ps] ***WARNING*** PLRS: Need to drive DQ0 and DQ3 to 1 !!! ", $time);
//tmp                         iPow_res = 0;
//tmp         //                abort = 1;
//tmp                         start_PLR2_rescue = 0;
//tmp                         disable Power_rescue_seq;
//tmp                     end
//tmp             iPow_res = iPow_res - 1;
//tmp             end else begin
//tmp                 -> plrs_part2_count_reached;
//tmp             `ifdef HOLD_pin 
//tmp                 release N25Qxxx.DQ0;
//tmp                 release N25Qxxx.HOLD_DQ3;
//tmp             `else 
//tmp                 release N25Qxxx.DQ0;
//tmp                 release N25Qxxx.RESET_DQ3;
//tmp             `endif
//tmp             //latchingMode = "N";
//tmp             end 
//tmp         end
//tmp     end
//tmp 
//tmp     // once enough count for part 2 of PLR Sequence have been reached
//tmp     // we wait for the rising edge of S and put bfm in extended.
//tmp     //---------------------------------------------------------------
//tmp     always @plrs_part2_count_reached begin
//tmp         @(posedge S) begin
//tmp             if(not_valid_2nd_part == 0) begin
//tmp                 iPow_res = 0;
//tmp                 start_PLR2_rescue = 0;
//tmp                 //power_rst = 0;
//tmp                 N25Qxxx.rescue_seq_flag = 0;
//tmp                 plr2_done = 1;
//tmp     //          VolatileEnhReg.VECR[5] = 1;
//tmp                 N25Qxxx.VolatileEnhReg.VECR[6] = 1;
//tmp                 N25Qxxx.VolatileEnhReg.VECR[7] = 1;    
//tmp     //            `ifdef MEDITERANEO
//tmp     //          N25Qxxx.DoubleTransferRate = !VolatileEnhReg.VECR[5];
//tmp     //            `endif
//tmp                 `ifdef INT_RST_SEQ
//tmp                     $display("  [%0t ps] ==INFO== PLRS: Rescue Sequence to reset the interface of memory device to power up state.", $time);
//tmp                 `else
//tmp                     if(!modify_in_progress) begin
//tmp                         $display("  [%0t ps] ==INFO== PLRS: 2nd part of Power Loss Rescue Sequence completed.", $time);
//tmp                         N25Qxxx.protocol = "extended";
//tmp                     end else begin
//tmp                         $display("  [%0t ps] ==WARNING== PLRS: Device Busy, 2nd part of Power Loss Rescue Sequence did not complete.", $time);
//tmp                         $display("  [%0t ps] ==WARNING== PLRS: Redo the 2nd part of the sequence or Redo the entire sequence.", $time);
//tmp                     end
//tmp                 `endif      
//tmp             end else begin
//tmp                 iPow_res = 7;
//tmp                 not_valid_2nd_part = 0;
//tmp                 plrs_part2_excess_clock = 0;
//tmp             end
//tmp         end
//tmp     end
//tmp 
//tmp     // checks for excess clock pulses once the required clock pulses
//tmp     // have been reached for the 2nd part of PLR Sequence
//tmp     //---------------------------------------------------------------
//tmp     always @plrs_part2_count_reached begin
//tmp         plrs_part2_excess_clock = plrs_part2_excess_clock + 1;
//tmp         if(plrs_part2_excess_clock > 1) begin
//tmp             if(not_valid_2nd_part == 0)
//tmp                 //$display("   [%0t ps] ==ERROR== PLRS: Excess clock cycles for 2nd part of PLRS ", $time);
//tmp             not_valid_2nd_part = 1;
//tmp         end    
//tmp     end
//tmp 
//tmp endmodule // PLRSpart2Detect module
